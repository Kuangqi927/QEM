...
def calculate_iqcm(tau):
    ...
    qcm_2 = normal_quantile.unsqueeze(2).cuda()
    qcm_1 = torch.ones(batch_size,num).unsqueeze(2).cuda()
    qcm_3 = (qcm_2.pow(2)-1)                                                                
    qcm_4 = (qcm_2.pow(3)-3*qcm_2)                                                             
    
    qcm = torch.cat([qcm_1,qcm_2,qcm_3,qcm_4],dim=2) #(batch,32,4)
    
    a = torch.bmm(qcm.transpose(1,2),qcm).inverse()
    b = torch.bmm(a,qcm.transpose(1,2))
...
def calculate_qcm(tau,V):
    V_inverse = V.inverse()
    tau = tau.squeeze().cpu().numpy()
    num = tau.shape[0]
    qcm_1 = torch.ones(num)
    qcm_2 = torch.ones(num)
    qcm_3 = torch.ones(num)
    qcm_4 = torch.ones(num)
    
    for i in range(num):
        normal_quantile=torch.FloatTensor(np.array(norm(0,1).ppf(tau[i])))
        qcm_2[i] = normal_quantile
        qcm_3[i] = normal_quantile.pow(2)-1
        qcm_4[i] = normal_quantile.pow(3)-3*normal_quantile
        
    qcm_t = torch.cat([qcm_1.unsqueeze(0),qcm_2.unsqueeze(0),qcm_3.unsqueeze(0),qcm_4.unsqueeze(0)],dim=0)
    qcm = qcm_t.T
    
    a = torch.mm(qcm_t.mm(V_inverse.mm(qcm)).inverse(),qcm_t)
