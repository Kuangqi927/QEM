...
class IQCM(BaseModel):
    ...
    def calculate_q(self, states=None, state_embeddings=None):
        ...
        qcm_weight = calculate_iqcm(taus).to(state_embeddings.device) #(batch_size, 4, self.K)
        ...
        #print(qcm_weight.shape,quantiles.shape,torch.bmm(qcm_weight,quantiles).shape)
        
        # Calculate expectations of value distributions.
        q = torch.bmm(qcm_weight,quantiles)[:,:2,:] #(batch_size, 1, self.num_actions)
