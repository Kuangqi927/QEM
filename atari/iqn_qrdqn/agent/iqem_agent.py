...
from iqn_qrdqn.model import IQEM
from iqn_qrdqn.utils import calculate_quantile_huber_loss, disable_gradients, evaluate_quantile_at_action, update_params

from .base_agent import BaseAgent


class IQEMAgent(BaseAgent):
    ...
    def calculate_loss(self, state_embeddings, actions, rewards, next_states,
        ...
        with torch.no_grad():
            ...
            if self.double_q_learning:
                ...
                self.online_net.sample_noise()
                next_q = self.online_net.calculate_q(states=next_states)[:,0,:]
            else:
                next_state_embeddings =\
                    self.target_net.calculate_state_embeddings(next_states)
                next_q = self.target_net.calculate_q(
                    state_embeddings=next_state_embeddings)[:,0,:]
