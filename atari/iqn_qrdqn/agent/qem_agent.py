...
from iqn_qrdqn.model import QEM
from iqn_qrdqn.utils import calculate_quantile_huber_loss, disable_gradients, evaluate_quantile_at_action, update_params,calculate_qem,LinearAnneaer

from .base_agent import BaseAgent


class QEMAgent(BaseAgent):

    def __init__(self, env, test_env, log_dir, explo,algo,num_steps=5*(10**7),
                 ...
                 epsilon_train=0.01, epsilon_eval=0.001,epsilon_start = 1.0,
                 epsilon_decay_steps=250000, double_q_learning=False,
                 dueling_net=False, noisy_net=False, use_per=False,
                 log_interval=100, eval_interval=250000, num_eval_steps=125000,
                 max_episode_steps=27000, grad_cliping=None, cuda=True,
                 seed=0,weight = 1.0):
        super(QEMAgent, self).__init__(
            env, test_env, log_dir,explo, algo, num_steps, batch_size, memory_size,
            gamma, multi_step, update_interval, target_update_interval,
            start_steps, epsilon_train, epsilon_eval,epsilon_start, epsilon_decay_steps,
        ...
        v[(self.tau_hats.squeeze()>=0.9)]=weight
        v[(self.tau_hats.squeeze()<=0.1)]=weight
        self.V = torch.diag(v)
        
        # N*N size
        self.qem_weight = calculate_qem(self.tau_hats,self.V).to(self.device)
        self.N = N
        
        #self.explo = explo
        #self.epsilon = LinearAnneaer(
            #1.0, 0, 6250000) #25M
        ...
        self.online_net = QEM(
            ...
            qem_weight=self.qem_weight, N=N, dueling_net=dueling_net,
    ...
    def calculate_loss(self, states, actions, rewards, next_states, dones,
        ...
        with torch.no_grad():
            ...
            if self.double_q_learning:
                ...
                next_ = self.target_net.calculate_q(states=next_states)
                next_q = next_[:,0,:]
                #next_var = next_[:,1,:]
                #next_skew = next_[:,2,:]
            ...
            #if self.explo == True:
                #self.epsilon.step()
                #next_q = next_q + self.epsilon.get() * next_skew.pow(1/3)
