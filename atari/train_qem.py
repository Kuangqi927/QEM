...
from iqn_qrdqn.agent import QEMAgent
...
def run(args):
    ...
    log_dir = os.path.join(
       log_name, 'QEM'+'-%s'%args.N+'-%s'%args.explo, args.env_id, f'{name}-seed{args.seed}-{time}')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--config', type=str, default=os.path.join('config', 'qem.yaml'))
