...
from .utils import LinearSchedule,calculate_qem,calculate_iqem
...
class QEMDSACTrainer(TorchTrainer):
    ...
    def __init__(
        ...
        presum_tau = ptu.zeros(256, self.num_quantiles) + 1. / self.num_quantiles
        ...
        v = torch.ones(self.num_quantiles,dtype=torch.float32)
        v[(tau_hat[0]>=0.9)]=weight
        v[(tau_hat[0]<=0.1)]=weight
        self.V = torch.diag(v).cuda()
        self.weight = calculate_qem(tau_hat,self.V)
    ...
    def train_from_torch(self, batch):
        ...
        if self.risk_type == 'VaR':
            ...
            if self.risk_type in ['neutral', 'std']:
                if self.tau_type == 'fix':
                    q1_new_actions = torch.matmul(z1_new_actions,self.weight.T)[:,:1]
                    q2_new_actions = torch.matmul(z2_new_actions,self.weight.T)[:,:1]
                elif self.tau_type == 'iqn':
                    weight_batch = calculate_iqem(new_tau_hat)
                    #print(weight_batch.shape,z1_new_actions.unsqueeze(2).shape) #(256,4,32)
                    q1_new_actions = torch.bmm(weight_batch,z1_new_actions.unsqueeze(2)).squeeze()[:,:1]
                    q2_new_actions = torch.bmm(weight_batch,z2_new_actions.unsqueeze(2)).squeeze()[:,:1]
                else:
                    raise ValueError('iqn or fix')
                #print(q1_new_actions.shape)
